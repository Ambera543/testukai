...
console.log("h2 tagai: ", myList.length - myListe.length);
...
document.querySelectorAll("div.prices h2")
...
document.getElementById("h1-color-back").addEventListener("click", function () {
  document.querySelector("h1").style.color = "green";
});
document.getElementById("h1-font-back").addEventListener("click", function () {
...
newAnimal.forEach((e) =>
  e.addEventListener("dblclick", () => (e.style.color = "red"))
);
...
animal.forEach((e) =>
  e.addEventListener("click", () => (e.style.fontSize = "130%"))
);
...
patinka.forEach((e) =>
  e.addEventListener("click", () => e.classList.replace("like-button", "like"))
);
...
const newSenjorai = document.createTextNode(
  "Senjorų grupė iki 10: tik 5.99 eur"
);
