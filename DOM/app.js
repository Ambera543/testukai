...
document
  .querySelectorAll("h2").forEach((element) => (element.style.color = "lightblue"));
...
// document.querySelectorAll("i").forEach((element) => (element.document.addEventListener("click", function () {
...
addEventListener("click", function () {document.querySelector(".prices").style.backgroundColor = "grey";
});
// d.	Padaryti, kad paspaudus ant tago su id contacts, tam tagui būtų pridėta css savybė color = orange;
document.querySelectorAll("#contacts").addEventListener("click", function(){document.querySelector(".div").style.color='orange'});
...
// document.querySelectorAll("li.new").addEventListener('dblclick', function(){document.querySelector("li.new").style.color = 'red'});
const cnew = document.querySelectorAll(".new");

for (let i = 0; i < cnew.length; i++) {
  cnew[i].addEventListener("dblclick", function () {
    cnew[i].classList.toggle("red");
  });
}
