...
if (pinigine1[i]< 3) {
 monetos.push(pinigine1[i])
  } else{(popieriniai.push(pinigine1[i])
  )};
}
...
for (let i = 0; i !== 14; i++){
let randomItem = mokejimoKorteles[(Math.floor(Math.random()*mokejimoKorteles.length))];
 korteles.push(randomItem);}
console.log(brangiPinigine);
console.log("13---------");
// 13. Paskaičiuokite, kokio tipo kortelių ('MasterCard' arba 'Visa') yra daugiau;
let MasterCard = korteles.filter(function(item){ return item === 'MasterCard'; }).length;
let Visa = korteles.filter(function(item){ return item === 'Visa'; }).length;
// if (MasterCard>Visa) {
//   return 'MasterCard'
// } else {return  'Visa'};
...
let bilietai =[];
for (let i = 0; i < 10; i++){
randomBilietai = Math.floor(Math.random()* (9999999999- 1000000000) + 1000000000)
...
bilietai.forEach(function (item, i) {
  if (item % 777 === 1) {
  console.log("You won", i)
  } else 
  return 
});
console.log("18---------");
